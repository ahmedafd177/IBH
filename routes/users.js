...
/* GET /api/users — accounts table merged with order stats */
router.get('/', (req, res) => {
  const accounts = db.prepare(
    'SELECT id, name, phone, email, role, created_at FROM accounts ORDER BY id ASC'
  ).all();
...
/* POST /api/users */
router.post('/', (req, res) => {
  const { name, phone, email = '', role = 'customer', password = 'ibh2025' } = req.body;
  if (!name || !phone) return res.status(400).json({ error: 'name and phone required' });
  try {
    const info = db.prepare(
      'INSERT INTO accounts (name, phone, email, role, password_hash) VALUES (?, ?, ?, ?, ?)'
    ).run(name, phone.trim(), email, role, hashPw(password));
    res.status(201).json({ id: info.lastInsertRowid, name, phone, email, role });
  } catch {
...
router.put('/:id', (req, res) => {
  ...
  db.prepare('UPDATE accounts SET name = ?, email = ?, role = ? WHERE id = ?')
    .run(name, email, role, req.params.id);
  res.json(db.prepare('SELECT id, name, phone, email, role FROM accounts WHERE id = ?')
    .get(req.params.id));
});
